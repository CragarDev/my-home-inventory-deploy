...
const Inventory = require("../server/models/inventory.model");
...
module.exports.createInventoryItem = async (req, res) => {
  ...
  if (file.size > 5000000) {
    deleteImage(_id);
    return res.status(400).send("File is too large, must be less than 5mb");
  }
  console.log("uploaded file", file);
  // return res.send(file.id);

  // req.body.inventoryImage = file.filename;
  ...
  Inventory.create(req.body) // req.file
    .then((newlyCreatedInventoryItem) => res.json({ results: newlyCreatedInventoryItem, message: "CREATING Inventory: Successful" }))
